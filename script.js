...
══════════════════════════════════ */
  (function () {
    ...
    const PF_WORKS = [
      ...
      {
        id:22, year:'2025',
        title:'メールマガジン・季節のご挨拶',
        category:'Email / Communication',
        tags:['メールマガジン','デジタル運用','ブランディング','ディスプレイ広告'],
        ...
        client:'ミカタ税理法人（ミカタグループ）',
        d:{
          overview:'※このページは枠（レイアウト）の確認用です。テキストは仮置きです。サービスのローンチや季節の節目に合わせて配信する、メールマガジン・季節のご挨拶の制作と配信運用を担当。バナーデザインから配信文のコピーライティングまで一貫して手がけました。',
          challenge:'（仮）メールマガジンは継続的に配信し続けることで初めて顧客との関係が育ちます。読み飛ばされず開封・閲覧してもらうための課題をここに記載します。',
          approach:'（仮）サービスや季節のテーマごとにビジュアルのトーンを設計し、一目で内容が伝わるバナーを制作。配信文の方針などをここに記載します。',
          creative:'（仮）クリスマスや暑中見舞いなど季節のご挨拶では、季節感とブランドの世界観を丁寧に表現。アニメーションを取り入れ、メール上でも目に留まる動きのある表現に仕上げています。',
          result:'（仮）成果のテキストをここに記載します。',
          role:'（仮）バナー・アニメーションのデザイン、配信文のコピーライティング、配信運用。'
        }
      },
    ];
