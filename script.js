  ...
  (function () {
    ...
    const PF_WORKS = [
      ...
      },
      {
        ...
        title:'オウンドメディア & コーポレートサイト運用',
        category:'Digital / Content',
        tags:['デジタル運用','ウェブデザイン','広報'],
        img:'img/works/work_13/work_13.webp',
        imgs: { a:true, a2:true, b:true, b2:true, c:true },
        client:'ミカタ税理法人（ミカタグループ）',
        d:{
          overview:'税理士法人の専門知識をオウンドメディアで継続発信し、SEOによる検索流入の増加を実現。記事・サムネイルの制作から代表コラムの整理・掲載、コーポレートサイトの継続運用までを一貫して担当し、会社の対外的な信頼向上に貢献したプロジェクト。',
          challenge:'税理士法人として専門知識を持つ一方、その知見が社外にほとんど届いていない状況でした。検索からの流入を増やし、新規顧客との接点を作るとともに、代表の考え方や会社の姿勢を言葉として発信することで、問い合わせ前から信頼を積み上げる場が必要でした。コーポレートサイトのお知らせ・事例・セミナー情報も、会社として一貫した形で継続的に更新できる体制が求められていました。',
          approach:'税制改正（定額減税・年収の壁など）や実務で迷いやすいテーマを中心に、検索されやすいテーマを選定（SEO）してコンテンツを継続的に発信しました。記事の流し込みにとどまらず、一覧での視認性を高めるためサムネイルを毎回デザイン制作。また代表コラム「柴田昇の眼」を設け、日々の経営への視点や考えを継続的に発信することで、専門知識だけでは伝わらない「人となり」を届ける場を作りました。あわせてコーポレートサイトにお知らせ・事例・セミナー情報を継続掲載し、オウンドメディアと連動した情報発信のハブとして運用しました。',
          ...
          result:'継続的なSEO発信により検索経由での流入増加に貢献。代表コラムは対外的な発信の柱となり、会社の考えを広く伝える媒体として機能しています。コーポレートサイトと連動させることで、属人的になりがちだった情報発信を会社として継続できる仕組みへと整えました。',
          role:'自社メディア・コーポレートサイトの運用全般。テーマ選定（SEO）、記事の流し込み・校正、図解・作図、サムネイルデザイン制作、代表コラムの整理・掲載、サイトのお知らせ・事例更新まで一貫して担当。'
        }
      },
    ];
