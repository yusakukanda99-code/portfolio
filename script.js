  ...
  (function () {
    ...
    const PF_WORKS = [
      ...
      },
      {
        ...
        client:'ミカタ税理法人（ミカタグループ）',
        d:{
          overview:'税理士法人ミカタの「会社設立」サービスにおける、Google広告・Yahoo!広告の継続運用案件。経営者層を主ターゲットに、競合の多いマーケットで安定的な獲得導線を維持。',
          challenge:'競合大手（クラウド会計サービス・大手士業ポータル等）が常時上位を占めるオークション環境で、自社サービスの差別化要素を訴求しながら、リード獲得を継続する必要がありました。',
          approach:'<ul><li><strong>LP整備：</strong> 新LP・旧LP の両系統で情報設計と訴求要素を継続調整</li><li><strong>広告アセット制作：</strong> RSA 見出し・説明文・カスタマイザ（地域名×サービス）を継続刷新</li><li><strong>テスト施策：</strong> 訴求パターンの A/B テストを継続実施</li><li><strong>地域別キャンペーン：</strong> 沖縄・多摩地区・静岡など、地域特化型キャンペーンの企画</li><li><strong>新フォーマット検証：</strong> P-MAX・AI Max など、新しい配信フォーマットの導入検証</li><li><strong>月次方針協議：</strong> 代理店の月次レポートを基に、次月施策の方向性を協議</li></ul>',
          creative:'<ul><li>経営者の意思決定段階に合わせた訴求文設計（コスト不安／手続き手間／設立後の運営支援等）</li><li>地域名カスタマイザを活用し、全国一律でなく地域文脈で響くクリエイティブを実装</li><li>新LPでは情報構造を再設計し、サービスの専門性（税理士法人）が他社と差別化される構成へ</li></ul>',
          result:'<ul><li>月間配信規模：インプレッション 約100万、CV 約120件のスケールで継続運用</li><li>多年にわたり多施策の横展開を継続し、変化する競合環境下でも安定供給を維持</li></ul>',
          role:'LP・広告クリエイティブ・アセット制作を担当。入札・配信運用は広告代理店、月次の改善方針協議に参画する分担体制で、デザインと施策設計の側面から継続貢献。'
        }
      },
      {
        id:10, year:'2026',
