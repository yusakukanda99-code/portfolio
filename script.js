    ...
    const PF_WORKS = [
      ...
      },
      {
        ...
        d:{
          ...
          result:'会社として情報を体系的・継続的にお客様へ届ける広報基盤を社内に構築。タイムリーな専門情報の発信により、既存顧客からの信頼と「頼れる専門家」としてのブランド認知を獲得し、属人的だった情報発信を仕組み化しました。<br><br><strong>運用実績</strong>※数値は更新中<ul class="d-list"><li><span class="d-label">配信数</span>約◯◯件／回</li><li><span class="d-label">配信頻度</span>月◯◯回（実益型・情緒型あわせて）</li><li><span class="d-label">平均開封率</span>◯◯％</li><li><span class="d-label">平均クリック率</span>◯◯％</li></ul>',
      ...
      },
      {
        ...
        client:'ミカタ税理法人（ミカタグループ）',
        d:{
          overview:'税務会計・経営コンサルティング領域における採用競争に勝つため、「ミカタで働くとはどういうことか」を求職者に具体的・魅力的に伝える採用ブランディングをゼロから構築。写真撮影・バナー制作・資料作成から採用サイトのディレクションまで一貫して担当したプロジェクト。',
          ...
          approach:'「経営者の意思決定を支える仕事」という仕事の本質的な魅力を一言で表すコンセプトを設定し、それを軸に資料・バナー・採用サイトの内容を整理しました。写真撮影は実際の社員を被写体として行い、リアルな働く姿を素材として確保。撮影にあたっては撮影スケジュール・移動時間・ロケーションを含む香盤表を作成し、当日の進行管理まで担当しました。オンラインセミナーの集客用バナーでは、タイポグラフィに強弱をつけ「意思決定」という言葉を視覚的に際立たせることで、一目で内容が伝わるデザインに仕上げました。',
          ...
          result:'採用セミナーへの参加申し込み獲得に貢献し、外部連携も含めた採用広報の基盤を整備。採用活動に使える写真素材を内製で確保したことで、今後の広告・資料制作のコストと工数の削減にも寄与しました。<br><br>▶ 採用サイト：<a href="https://mikata-c.co.jp/recruit.html" target="_blank" rel="noopener">https://mikata-c.co.jp/recruit.html</a>',
          role:'採用広報の企画・制作全般。写真撮影・選定（香盤表作成・撮影進行管理を含む）、セミナー集客用バナー制作、採用関連資料の作成を担当。採用サイトではコンテンツ作成のディレクション（一部は自ら制作）と、サイト全体のデザインチェック（トーン・品質の統一）を担当。'
        }
      },
      {
        id:24, year:'2026',
        ...
        client:'社内向けプロダクト（現職）',
        d:{
          overview:'社外サービスへの依存、AIがファイルを直接読めない、顧客納品時のPDF変換の手間——こうした社内のドキュメント業務の課題に対し、現場調査からPMF検証、UI/UXデザイン、実装までを一人で担当し、ブラウザだけで完結する自社PDF編集ツールを開発した制作物です。',
          challenge:'ドキュメント業務では、PDF編集のたびに社外の有料サービスへ依存し、コストとセキュリティの両面で課題がありました。また、AIがファイルを直接読み込めない、顧客への納品時にPDF変換の手間がかかるといった、日々の小さな非効率が積み重なっていました。',
          approach:'現場で実際の困りごとをヒアリングし、三島支店で試用してもらいながら「本当に使われる形」を検証（PMF）。UI/UXは専門知識がない人でも迷わないことを最優先に設計しました。技術的にはブラウザ完結（pdf.js / fabric.js / jsPDF）で実装し、AIを活用して高速に開発。現場の声を起点に、ドラッグ&ドロップでのPDF結合・ページ回転・Undo/Redoなどを継続的に追加していきました。',
          creative:'専門用語を避けることに徹しました。たとえば保存時の解像度（dpi）は「印刷用・画面用・メール用」という"用途の言葉"に翻訳し、誰でも目的から選べる保存UXに。電子押印も、法人名・部署名・氏名を自由に設定できるよう、日本のビジネス実務に合わせた形にしました。',
          result:'Adobe Acrobat の導入費と比較し、年間約1,200万円のコスト削減効果を試算。現在、社内標準ツールとして導入を提案しています。「課題発見 → 企画 → 制作 → 効果測定」というマーケティングと共通の一貫プロセスを単独で完遂し、施策の価値を数値で示せることを実証した一枚です。<br><br>▶ デモ：<a href="https://pdf-annotator-mycompany.netlify.app/" target="_blank" rel="noopener">https://pdf-annotator-mycompany.netlify.app/</a>',
          role:'企画・デザイン・開発を単独で担当。現場ヒアリングとPMF検証、UI/UXデザイン、フロントエンド実装（pdf.js / fabric.js / jsPDF）、AIを活用した開発、社内導入提案。'
        }
      },
      {
        id:4, year:'2023–2026',
        ...
        d:{
          ...
          creative:'税務・労務という目に見えないサービスを扱うため、金融機関や信頼感を連想させる背景画像を取り入れながら、文字を大きく使った力強いデザインで表現しました。年に複数回開催するため、毎回似た印象にならないよう変化をつけながらも、重厚感と信頼感は一貫して守るよう丁寧に調整。参加者に「大切にされている」と感じてもらえるよう、細かな心配りを随所に盛り込みました。<br><br>セミナー集客ページ：<a href="https://mikata-c.co.jp/seminar/250912.html" target="_blank" rel="noopener">https://mikata-c.co.jp/seminar/250912.html</a>',
          result:'東京・大阪・名古屋・鹿児島・沖縄・軽井沢・六本木など全国各地で満員御礼を達成し、多くの新規見込み客を獲得。\\n\\n【セミナー運営の実績】\\n2023〜2025年 東京（各300名程度）・大阪（各300名程度）\\n2024〜2025年 名古屋（各80名程度）\\n2024年 ミカタグループ創立30周年パーティ（700名程度）\\n2025年 鹿児島（100名）・六本木（90名）・沖縄（100名）・軽井沢（50名）※招待制により非公開',
      ...
      },
      {
        ...
        tags:['ウェブデザイン','ブランディング','プライベート'],
        img:'img/works/atelier_prive/atelier_prive.webp',
        client:'個人制作プロジェクト',
        d:{
          overview:'「"探す"を"出会う"に変える」という体験設計の思想のもと、個人制作プロジェクトとして設計・実装した、招待制のアート体験の場（Webアプリケーション）。コンセプト設計からUIデザイン、フロントエンド・バックエンド実装、AI機能までを一人で手がけた制作物です。',
          challenge:'アートを「自分から探しに行く」ものではなく、日常の中で自然に「出会う」ものにできないか、という問いを起点にしました。鑑賞の心理的なハードルや、作品との偶発的な出会いの少なさという体験上の課題に対し、UIと情報設計の両面から応える必要がありました。',
          approach:'コンセプトを軸に、独自のカラー体系とUIを設計。フロントエンドをNext.js、バックエンド（認証・データ）をSupabaseで実装し、招待制の閲覧フローを構築しました。AIを活用した機能も設計・実装し、作品との出会い方そのものを探求。あわせて、作品画像が無断で利用されないための保護の仕組みも設計に組み込みました。デザインと実装を分断せず一人で地続きに手を動かすことで、思想を細部まで反映させました。',
          creative:'「アートが日常に滲み出す」という世界観を、余白とタイポグラフィ、独自のカラー体系で表現しました。"探す"のではなく"出会う"導線を意識し、ユーザーが構えずに作品と向き合えるUIを設計。閲覧体験のトーンと、ブランドとしての一貫した佇まいを両立させました。',
          result:'デザイン（UI・ブランド）とフルスタック実装、AIの活用までを一人で地続きに行った制作物として、企画から技術実装までを横断する設計力を形にしました。コンセプトを"動くもの"として体験できる状態まで落とし込んだ取り組みです。',
          role:'個人制作（全工程を単独担当）。コンセプト・体験設計、UI・カラー体系のデザイン、フロントエンド（Next.js）・バックエンド（Supabase）実装、AI機能の設計・実装、作品画像の保護設計。'
        }
      },
      {
        id:9, year:'2024',
      ...
      },
      {
        ...
        client:'ミカタ税理法人（ミカタグループ）',
        d:{
          overview:'創業30周年を迎えるミカタグループに向けて、会社案内（全16ページ）・営業チラシ・各種資材をトータルで統一制作。紙の会社案内から社章（ブランドエンブレム）まで、戦略立案・フォトディレクション・作図・ライティング・デザイン・3Dモデリングまでを社内で一貫して完結させた大型プロジェクト。',
          ...
          approach:'代表挨拶の写真撮影ディレクションから、全ページの作図・デザイン・ライティングまでを一貫して担当。会社案内（全16ページ）では、ミッション・ビジョン・バリューの体系的な整理から始め、「ビジネス＆ライフ・コンサルティング」の独自性をわかりやすく図解化（法人・個人の2つの財布の可視化、6ステップの成長支援ロードマップ、4つのソリューション体系など）。営業チラシは、ターゲット（中小企業オーナー）の課題感に直結したキャッチコピー設計と、フローチャート・Q&Aを用いた分かりやすい情報設計を採用しました。あわせて、ブランドの象徴となる社章を3DソフトMayaでモデリング＆レンダリングし、完成イメージを高精度に可視化した上で経営層へ提案しました。',
          creative:'代表の写真撮影では、「経営者のパートナー」という信頼感を伝えるため、表情・姿勢・照明・背景まで細かくディレクション。会社案内の表紙は「山を登る経営者たち」という墨絵風のイラストを採用し、「ひ孫の世代まで伴走する」というブランドの世界観を視覚的に表現しました。コーポレートカラーの黒を軸に、グレーのグラデーション・差し色の赤を精緻に調整し、重厚感と親しみやすさが共存するデザインシステムを構築。社章は金・銀の縁取りや立体加工を検討し、「プロフェッショナル」「信頼感」「高級感」を兼ね備えたエンブレムに仕上げ、紙の資材から身に着けるモノまで一貫したブランド体験を設計しました。',
          ...
          role:'クリエイティブディレクター（全工程を単独担当）。代表撮影のフォトディレクション、全ページのデザイン・作図・ライティング、カラー・フォントの選定・管理、営業チラシのコンテンツ設計、社章の3Dモデリング（Maya）。'
        }
      },
      {
        id:3, year:'2026',
